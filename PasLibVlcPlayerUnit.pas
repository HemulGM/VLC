...
procedure TPasLibVlcPlayer.SetFullScreen(aValue: Boolean);
begin
  if (p_mi = NIL) then
    exit;
  if aValue then
...
function TPasLibVlcPlayer.GetHideCursor: Boolean;
begin
  Result := FMouseEventWinCtrl.Cursor = crNone;
end;
...
procedure TPasLibVlcPlayer.SetHideCursor(const Value: Boolean);
begin
  if Value then
  begin
    FMouseEventWinCtrl.Cursor := crNone;
    Cursor := crNone;
  end
  else
  begin
    FMouseEventWinCtrl.Cursor := crDefault;
    Cursor := crDefault;
  end;
end;
...
function TPasLibVlcPlayer.GetVideoLenInMsAlter: Int64;
begin
  if (GetVideoPosInMs = 0) or (GetVideoPosInPercent = 0) then
    Exit(0);
  Result := Trunc((GetVideoPosInMs / GetVideoPosInPercent) * 100);
end;

function TPasLibVlcPlayer.GetVideoLeftInMs: Int64;
begin
  Result := Max(0, GetVideoLenInMsAlter - GetVideoPosInMs);
end;
